...
def batchnorm_backward(dout, cache):
  ...
  dx_hat = gamma * dout
  dx_2_b = np.sum((x - sample_mean) * dx_hat, axis=0)
  dx_2_a = ((sample_var+ eps) ** -0.5) * dx_hat
...
def svm_loss(x, y):
  ...
  # print correct_class_scores.shape (N,)
  margins = np.maximum(0, x - correct_class_scores[:, np.newaxis] + 1.0)
  margins[np.arange(N), y] = 0
  loss = np.sum(margins) / N
  rows_pos = np.sum(margins > 0, axis=1)
  dx = np.zeros_like(x)
  dx[margins > 0] = 1
  dx[np.arange(N), y] -= rows_pos
  dx /= N
  return loss, dx
